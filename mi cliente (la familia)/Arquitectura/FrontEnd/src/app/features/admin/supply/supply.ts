...
export class AdminSupply implements OnInit, AfterViewInit {
  ...
  private initDatePickers() {
    ...
    // Inicializar date picker para fecha de orden
    ...
    // Inicializar date pickers para fechas de caducidad (los que tengan id 'expiry*')
    document.querySelectorAll('[id^="expiry"]').forEach(el => {
      ...
      const instance = flatpickr(input, {
        ...
        allowInput: true,
        onChange: (selectedDates, dateStr) => {
          // Extraer el ID del producto del input ID (formato: expiry{productId})
  ...
  openOrder() {
    this.orderDraft = { supplier_id: this.suppliers()[0]?.id || '', expected_date: '', notes: '' };
