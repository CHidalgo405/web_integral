...
export class AdminFinance implements OnInit, AfterViewInit {
  ...
  private initDatePickers() {
    ...
    // Inicializar date picker para fecha de gasto
    const expenseDateInput = document.getElementById('expenseDate') as HTMLInputElement | null;
    if (expenseDateInput) {
      const instance = flatpickr(expenseDateInput, {
        dateFormat: "Y-m-d",
        locale: "es",
        allowInput: true,
        onChange: (selectedDates, dateStr) => {
          this.draft.expense_date = dateStr;
        }
      });
  ...
  openExpense() {
    this.draft = { category_id: this.categories()[0]?.id || '', description: '', amount: 0, payment_method: 'cash', receipt_ref: '', expense_date: new Date().toISOString().slice(0, 10) };
